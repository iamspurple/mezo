...
const initModalScrollbarPadding = () => {
  ...
  function getScrollbarWidth() {
    if (cachedWidth == null) {
      cachedWidth = measureScrollbarWidth();
    }
    return cachedWidth;
  }

  function syncModalScrollbarPadding() {
    const hasOpenModal = document.querySelector(".modal:not(.is-hidden)");
    const w = hasOpenModal ? getScrollbarWidth() : 0;
  ...
  function init() {
    syncModalScrollbarPadding();

    const observer = new MutationObserver(syncModalScrollbarPadding);
